...
export default function AddCategory() {
  ...
  const myCategoryForm = () => (
    <form className="mx-auto">
      <div className="w-9/12 mx-auto px-12 py-4">
        ...
        />
        <button
          className="my-2 px-4 py-2 border border-cyan-600 focus:outline-none focus:ring focus:border-none  text-cyan-600 rounded"
          onClick={onSubmit}
        >
          Create Category
        </button>
  ...
  const goToDashboard = () => (
    <div className="px-2 pt-5">
      <Link
        to="/admin/dashboard"
        className="bg-gray-600 text-sm px-4 py-2 text-white border border-transparent transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-gray-700 duration-300"
      >
        Admin Home
      </Link>
    </div>
